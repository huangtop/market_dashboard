...
def get_json(
    ...
    """Fetch a TWSE JSON endpoint with conservative retry/backoff.

    TWSE occasionally returns an empty body or an HTML throttling/error page
    with HTTP 200.  Treat those as retryable failures instead of allowing
    ``response.json()`` to abort a long backfill immediately.
    """
